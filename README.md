...
A dark "Vibrant Scholastic" portfolio built with [Astro](https://astro.build) and Tailwind CSS, deployed to GitHub Pages.
...
- `src/components/` — Nav, Hero, StatsBar, Portfolio, BlogList, Footer
...
- `src/styles/global.css` — theme colour variables (`:root`)
- `tailwind.config.mjs` — design tokens from `DESIGN.md`
- `public/` — static assets (`files/cv.pdf`, `videos/hero.mp4`, `images/`)

## Theming

Dark only. Colours are CSS variables consumed by Tailwind.
